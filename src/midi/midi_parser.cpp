    ...
    MidiDocument& output)
{
    ...
    while (cursor.remaining() != 0 && ok) {
        ...
        // SharpMIDI stores (track << 4) in a single byte beside channel, so
        // renderer ownership/color wraps every 16 tracks. Keep that exact
        // compact identity in the runtime event stream. The legacy MPWGL2
        // VisualNote builder still computes its own appearance palette below.
        const uint8_t trackColor =
            static_cast<uint8_t>(trackIndex & 0x0f);
    ...
    const std::vector<TrackScan>& scans)
{
    ...
    for (const TickGroup& group :
        ...
        for (std::size_t eventIndex =
            ...
            // Do not rewrite CompactEvent's high nibble here. The new
            // SharpMIDI renderer uses it as track-id modulo 16, exactly like
            // upstream's byte-sized (track << 4). Legacy VisualNotes below use
            // perTrackColor directly and therefore keep their old palette.
...
void buildDerivedStats(MidiDocument& output)
{
    ...
    output.derivedCcTimeline.clear();
    ...
    if (output.durationSeconds > 0.0f) {
        ...
        output.derivedCcTimeline.assign(bucketCount, 0);

        for (const TickGroup& group : output.tickGroups) {
            if (group.noteOnCount == 0 && group.controlCount == 0)
                continue;
            ...
            if (group.controlCount != 0) {
                // TickGroup::controlCount is already the exact count of
                // B/C/D/E channel-state events. Reuse it instead of rescanning
                // the event payload, and importantly do not make CC-only ticks
                // depend on noteOnCount. The previous placement after the
                // note-only early exit left the CC/s mountain graph dead for
                // controller-dense files with no NoteOn on the same tick.
                const uint64_t ccSum =
                    uint64_t(output.derivedCcTimeline[bucket]) + group.controlCount;
                output.derivedCcTimeline[bucket] = static_cast<uint32_t>(
                    std::min<uint64_t>(ccSum, std::numeric_limits<uint32_t>::max()));
            }
        }
    }
    ...
    const char*& errorMessage)
{
    ...
    report(90, "Finalizing timing metadata");
    // Runtime graphs are sampled during playback; do not add a third
    // all-song statistics pass to MIDI loading.
    output.derivedNpsTimeline.clear();
    output.derivedCcTimeline.clear();
    output.derivedPeakNps = 0;
    output.derivedPeakNpsTime = 0.0f;
    output.derivedPeakPolyphony = 0;
    output.derivedStatsReady = false;
