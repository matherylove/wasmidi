...
bool readStatus(const uint8_t*& p, const uint8_t* end, uint8_t& runningStatus, uint8_t& status)
{
    if (p >= end)
        return false;
    status = *p++;
...
bool MidiParser::parse(const uint8_t* data, std::size_t size, MidiDocument& output)
{
    ...
    if (std::memcmp(p, "MThd", 4) != 0) { errorMessage_ = "Missing MThd header"; return false; }
    p += 4;
    const uint32_t headerSize = readU32BE(p, end, ok);
    if (!ok || headerSize < 6 || std::size_t(end - p) < headerSize) { errorMessage_ = "Invalid MIDI header"; return false; }
    ...
    if (output.ticksPerBeat == 0 || (output.ticksPerBeat & 0x8000)) { errorMessage_ = "SMPTE timing is not supported"; return false; }
    p = data + 8 + headerSize;

    std::vector<TrackRef> tracks;
    tracks.reserve(output.trackCount);
    for (uint16_t i = 0; i < output.trackCount; ++i) {
        if (end - p < 8 || std::memcmp(p, "MTrk", 4) != 0) { errorMessage_ = "Invalid or truncated MTrk chunk"; return false; }
        p += 4;
        const uint32_t len = readU32BE(p, end, ok);
        if (!ok || std::size_t(end - p) < len) { errorMessage_ = "Truncated track data"; return false; }
    ...
    // Pass 1: collect the complete tempo map before converting any event to seconds.
    for (const auto& track : tracks) {
        ...
        while (tp < te && ok) {
            ...
            uint8_t status = 0;
            if (!ok || !readStatus(tp, te, running, status)) { ok = false; break; }
            ...
            if (status == 0xff) {
                ...
                if (!ok || std::size_t(te - tp) < len) { ok = false; break; }
                if (meta == 0x51 && len >= 3) {
                    const uint32_t uspb = (uint32_t(tp[0]) << 16) | (uint32_t(tp[1]) << 8) | tp[2];
            ...
            if (status < 0xf0) {
                if (!skipChannelMessage(tp, te, status)) { ok = false; break; }
                continue;
            }
            ok = false;
        }
    ...
    std::stable_sort(output.tempoMap.begin(), output.tempoMap.end(), [](const TempoChange& a, const TempoChange& b) { return a.tick < b.tick; });
    ...
    // Pass 2: notes + all channel events, now using the final tempo map.
    for (std::size_t trackIndex = 0; trackIndex < tracks.size(); ++trackIndex) {
        ...
        while (tp < te && ok) {
            ...
            if (status >= 0xf0) { ok = false; break; }

            const uint8_t command = status & 0xf0;
            const uint8_t channel = status & 0x0f;
            const int count = (command == 0xc0 || command == 0xd0) ? 1 : 2;
            if (te - tp < count) { ok = false; break; }
            const uint8_t d1 = *tp++;
            const uint8_t d2 = count == 2 ? *tp++ : 0;
            const float seconds = static_cast<float>(tickToSeconds(tick, output.tempoMap, output.ticksPerBeat));
            ...
            if (command == 0x80 || (command == 0x90 && d2 == 0)) {
                ...
                if (it != pending.end() && !it->second.empty()) {
                    ...
                    NoteEvent note;
                    note.startTime = static_cast<float>(tickToSeconds(start.tick, output.tempoMap, output.ticksPerBeat));
                    note.endTime = seconds <= note.startTime ? note.startTime + 0.015f : seconds;
                    note.pitch = d1; note.channel = channel; note.velocity = start.velocity; note.track = static_cast<uint16_t>(trackIndex);
                    output.notes.push_back(note);
                }
                continue;
            }

            if (command == 0xa0 || command == 0xb0 || command == 0xc0 || command == 0xd0 || command == 0xe0)
                output.controls.push_back({seconds, command, channel, d1, d2});
        }

        const float trackEnd = static_cast<float>(tickToSeconds(tick, output.tempoMap, output.ticksPerBeat));
        for (auto& [key, queue] : pending) {
            while (!queue.empty()) {
                const PendingNote start = queue.front(); queue.pop_front();
                NoteEvent note;
                note.startTime = static_cast<float>(tickToSeconds(start.tick, output.tempoMap, output.ticksPerBeat));
                note.endTime = trackEnd <= note.startTime ? note.startTime + 0.015f : trackEnd;
    ...
    std::sort(output.notes.begin(), output.notes.end(), [](const NoteEvent& a, const NoteEvent& b) { return a.startTime < b.startTime; });
    std::sort(output.controls.begin(), output.controls.end(), [](const ControlEvent& a, const ControlEvent& b) { return a.time < b.time; });
    for (const auto& note : output.notes)
        output.durationSeconds = std::max(output.durationSeconds, note.endTime);

    errorMessage_ = "";
    return true;
}

double MidiParser::tickToSeconds(uint32_t tick, const std::vector<TempoChange>& tempoMap, uint16_t ppq) const
{
    if (tempoMap.empty() || ppq == 0)
        return 0.0;

    const auto upper = std::upper_bound(tempoMap.begin(), tempoMap.end(), tick,
        [](uint32_t value, const TempoChange& t) { return value < t.tick; });
    const std::size_t index = upper == tempoMap.begin() ? 0 : static_cast<std::size_t>((upper - tempoMap.begin()) - 1);

    double seconds = 0.0;
    for (std::size_t i = 1; i <= index; ++i) {
        const auto& prev = tempoMap[i - 1];
        const auto& cur = tempoMap[i];
        seconds += (double(cur.tick - prev.tick) / double(ppq)) * (double(prev.microsecondsPerBeat) / 1'000'000.0);
    }
    const auto& base = tempoMap[index];
    seconds += (double(tick - base.tick) / double(ppq)) * (double(base.microsecondsPerBeat) / 1'000'000.0);
    return seconds;
}
